...
class MenuController extends Controller
{
    public function index()
    {
        ...
        return view('admin.menu.index', ['posts' => News::all(), 'pages' => Page::all(), 'services' => Services::all(), 'abroadss' => Abroad::all(), 'menus' => MenuLocation::all(), 'desiredMenu' => $desiredMenu, 'menuitems' => $menuitems]);
    }
    ...
    public function addabroadsToMenu(Request $request)
    {
        $data = $request->all();
        $menuid = $request->menuid;
        $ids = $request->ids;
        ...
        if ($menu->content == '') {
            foreach ($ids as $id) {
                $data['title'] = Abroad::where('id', $id)->value('name');
                $data['slug'] = '/abroad-studies/' . Abroad::where('id', $id)->value('slug');
                $data['type'] = 'abroads';
                $data['menu_id'] = $menuid;
                $data['updated_at'] = NULL;
                Menu::create($data);
            ...
            foreach ($ids as $id) {
                $array['title'] = Abroad::where('id', $id)->value('name');
                $array['slug'] = '/abroad-studies/' . Abroad::where('id', $id)->value('slug');
                $array['name'] = NULL;
                $array['type'] = 'abroads';
                $array['target'] = NULL;
                $array['id'] = Menu::where('slug', $array['slug'])->where('name', $array['name'])->where('type', $array['type'])->orderby('id', 'DESC')->value('id');
