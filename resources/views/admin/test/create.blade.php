    ...
    <div class="content">
        <div class="card container-fluid mb-4 p-6">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h3 class="mb-0">Create-TestPreperation</h3>
                <small class="text-muted float-end">
                    <a href="{{ route('test.index') }}" class="btn btn-primary"><i class="fa-solid fa-arrow-left"></i>
                        Back</a>
                </small>
            </div>

            <div class="card-body p-0">
                <form method="POST" class="row" action="{{ route('test.store') }}" enctype="multipart/form-data">
                    @csrf
                    <!-- functions -->
                    @php
                        $fields = [
                        ...
                        <div class="card-body card font-weight-bold br-8 mb-3">
                            ...
                            @foreach ($fields as $name => $label)
                                <div class="form-group font-weight-bold mb-3">
                                    ...
                                    @elseif ($name == 'additional_description')
                                        <textarea name="{{ $name }}" id="{{ $name }}" rows="10"
                                            class="form-control ckeditor2 br-8 @error($name) is-invalid @enderror"
                                            placeholder="Enter {{ strtolower($label) }}">{{ old($name) }}</textarea>
