...
@section('title', 'All Test Preperation')

@section('content')
    @include('admin.includes.message')
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h3 class="mb-0 text-capitalize font-weight-bold ">Test Preperations ({{ $test->total() }})</h3>
            <small class="text-muted float-end">
                <a href="{{ route('test.create') }}" class="btn btn-primary"><i class="fa-solid fa-plus"></i>
                    Create</a>
            </small>
        </div>
            ...
            @if (!$test->isEmpty())
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach ($test as $key => $blog)
                            <tr>
                                <td><strong>{{ $key + $test->firstItem() }}</strong></td>
                                <td class="">
