...
@section('title', 'All abroads')

@section('content')
    @include('admin.includes.message')
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">abroads ({{ $abroads->total() }})</h5>
            <small class="text-muted float-end">
                <a href="{{ route('abroads.create') }}" class="btn btn-primary"><i class="fa-solid fa-plus"></i>
                    Create</a>
            </small>
        </div>
                        ...
                        @foreach ($abroads as $key => $blog)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="{{ route('abroads.edit', $blog->id) }}"
                                        style="float: left;margin-right: 5px;" class="btn btn-sm btn-primary"><i
                                            class="fa-solid fa-pen-to-square"></i> Edit</a>


                                    <form class="delete-form" action="{{ route('abroads.destroy', $blog->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
